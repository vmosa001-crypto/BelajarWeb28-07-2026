...
async function getOrders() {
  ...
  return (res.data.values || []).map(row => ({
    ...
    address: row[5], city: row[6], items: row[7], total: Number(row[8]), status: row[9] || 'Pending'
  }));
}

module.exports = { getProducts, getProduct, addOrder, getOrders, isSheetsConfigured };
